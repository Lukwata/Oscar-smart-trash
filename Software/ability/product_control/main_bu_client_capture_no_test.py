...
def capturecc(cam):
    ...
    timestr = ""
    if cam == "0":
        timestr= time.strftime("%d%m%Y-%H%M%S") + "_C1"
    if cam == "1":
        timestr= time.strftime("%d%m%Y-%H%M%S") + "_C2"

    os.system("sudo fswebcam -d /dev/video"+cam+" -r 640x320 --no-banner /home/pi/aos/data/saveIMG/"+timestr+".jpg")
    #print( time.time()-t)
    #print ("done")
    if cam == "0":
        DONE_CAM0=True
            ...
            if self.event.wait(1):
                try:
                    ...
                    if (detect_count==0 or detect_count >=MAX_TIME_DELAY) and message!=None and message["ID"]=="COVER_STATUS" and message["Value"]==1: 
                        ...
                        while cam1.query_image():
                            image1= cam1.get_image()
                        while cam2.query_image():
                            image2= cam2.get_image()

                        timestr= time.strftime("%d%m%Y-%H%M%S")
                        timestrCam1= "/home/pi/aos/data/saveIMG/"+timestr + "_C1" + ".jpg"
                        timestrCam2= "/home/pi/aos/data/saveIMG/"+timestr + "_C2" + ".jpg"

                        pygame.image.save(image1, timestrCam1)
                        pygame.image.save(image2, timestrCam2)
                        
                        timestrCam0 = "/home/pi/aos/data/saveIMG/"+timestr + "_C0" + ".jpg"
                        pil.save(timestrCam0, format='JPEG')
