...
while [ ! -f .done ]; do
	...
	if [ "$ip" == "" ]; then
		...
		tmux send-keys -t navi:demo.0 'sleep 5; source ~/set_params.sh; roscore' C-m
		tmux send-keys -t navi:demo.1 'sleep 10; source ~/set_params.sh; roslaunch navi_services navi_services.launch' C-m
		tmux send-keys -t navi:demo.2 'sleep 15; source ~/set_params.sh; roscd navi_services; cd src; python navi_bridge.py' C-m
		tmux send-keys -t navi:demo.3 'sleep 15; source ~/set_params.sh; roslaunch pr_teleop pr_move_service.launch' C-m
