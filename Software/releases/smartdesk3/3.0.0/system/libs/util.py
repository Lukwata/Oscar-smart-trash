...
from aos.system.configs.channel import CURRENT_PATH, DEVICE_TYPE, BRAIN_CONFIG_PATH, PATH_WIFI_CONFIG, \
    PATH_PRODUCT_ID_CONFIG, PATH_USER_CONFIG, PATH_PRODUCT_KEY_CONFIG, PATH_PRODUCT_ADDRESS
...
class Util(object):
    ...
    def get_product_key():
        ...
        product_key = ''
        result = Util.read_file(PATH_PRODUCT_KEY_CONFIG)
        if result is not None:
    ...
    @staticmethod
    def get_product_address_config():
        return Util.read_file(PATH_PRODUCT_ADDRESS)
    ...
    def get_user_data():
        try:
            return Util.read_file(PATH_USER_CONFIG)
        except IOError as e:
            print e.message
            pass
        except Exception as e:
    ...
    def get_time_zone():
        ...
        result = Util.read_file(PATH_WIFI_CONFIG)
        if result:
            time_zone = result['time_zone'] if 'time_zone' in result else result['timezone']
        return time_zone
    ...
    @staticmethod
    def set_date_time(time_zone=None):
        try:
            #list commmad:
            if not time_zone:
                time_zone = Util.get_time_zone()
            Util.cmd('sudo rm /etc/localtime')
            Util.cmd("sudo ln -s /usr/share/zoneinfo/%s /etc/localtime" % time_zone)
    ...
    def check_invalid_data(data_string):
        key_array = ['action', 'source', 'verify_code', 'address_long', 'address_lat', 'time_zone', 'user_id', 'wpa', 'ssid', 'user_hash', 'product_name', 'product_type', 'token']
        result = Util.Result()
        try:
    ...
    def update_pr_device_id(product_id, firebase_uid):
        ...
        if json_data and product_id:
            ...
            json_data["firebase"]['uid'] = firebase_uid
    ...
    @staticmethod
    def parse_variables(group_variables):
        group_variable = group_variables[0]
        if "task_variable_value" not in group_variable:
            return {}
        variables = group_variable["task_variable_value"]
        new_variables = {}
